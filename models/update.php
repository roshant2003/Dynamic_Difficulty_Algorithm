...
//delete user
if (isset($_SESSION['key'])) {
  if (@$_GET['demail'] && $_SESSION['key'] == 'AD') {
    $demail = @$_GET['demail'];
...
if (isset($_SESSION['key'])) {
  if (@$_GET['demail1'] && $_SESSION['key'] == 'AD') {
...
//remove quiz
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'rmquiz' && $_SESSION['key'] == 'AD') {
    $eid = @$_GET['eid'];
    ...
    header("location:../views/dash.php?q=5");
  }
}

//add quiz
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'AD') {
    $name = $_POST['name'];
    ...
    header("location:../views/dash.php?q=4&step=2&eid=$id&n=$total");
  }
}

//add question
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == 'AD') {
    $n = @$_GET['n'];
    $eid = @$_GET['eid'];
    $ch = @$_GET['ch'];
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
  ...
  if ($sn != $total) {
    $sn++;
    header("location:../views/account.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total") or die('Error152');
  } else if ($_SESSION['key'] != 'AD') {
    ...
    header("location:../views/account.php?q=result&eid=$eid");
  } else {
    header("location:../views/account.php?q=result&eid=$eid");
  }
}
...
if (@$_GET['q'] == 'quizre' && @$_GET['step'] == 25) {
  ...
  $sun = $sun - $s;
  $q = mysqli_query($con, "UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'") or die('Error174');
  header("location:../views/account.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
